...
#ifndef APEX_INPUTMANAGER_H
#define APEX_INPUTMANAGER_H
#include <libinput.h>
#include <iostream>
#include <vector>
#include <algorithm>
    ...
    static std::vector<std::string> listInputDevices(const char* deviceType) {
        ...
        udev_list_entry_foreach(device, devicesList) {
            ...
            if (dev) {
                ...
                if (devnode != nullptr && isValidInputDevice(devnode)) {
...
public:
    ...
    static void MoveMouseAbosule(int x, int y) {
...
Display* InputManager::display = nullptr;

#endif //APEX_INPUTMANAGER_H
